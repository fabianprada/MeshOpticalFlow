...
#pragma omp parallel for
	...
	template< class Real >
	void solveTS( ConstPointer( Real ) b , Pointer( Real ) x ) const
	{
		Eigen_Vector eigenB( _eigenM.rows() );
#pragma omp parallel for
		for( int i=0 ; i<eigenB.size() ; i++ ) eigenB[i] = b[i];
		Eigen_Vector eigenX = _solver.solve( eigenB );
#pragma omp parallel for
		for( int i=0 ; i<eigenX.size() ; i++ ) x[i] = (Real)eigenX[i];
	}
...
template< class Real , class MatrixRowIterator > CholmodSolver::CholmodSolver( const SparseMatrixInterface< Real , MatrixRowIterator >& M ){ _init( M ) , _update( M ); }
template< class Real , class MatrixRowIterator >
...
template< class Real , class MatrixRowIterator >
bool CholmodSolver::_update( const SparseMatrixInterface< Real , MatrixRowIterator >& M )
{
